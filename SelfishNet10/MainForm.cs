...
namespace SelfishNet
{
    public partial class MainForm : Form
    {
        ...
        private void EndResolvCallBack(IAsyncResult re)
        {
            ...
            updateTreeViewNameCallBack(asyncState, hostName);
        }
        ...
        private void updateTreeViewNameCallBack(PC pc, string str)
        {
            if (!pc.isGateway)
            {
                ...
                if (1 < this.treeGridView1.Nodes[0].Nodes.Count)
                {
                    ...
                    InvokeTreeGridViewText(treeGridView1, str);
                }
            }
            else
            {
                ...
                //treeGridView1.Nodes[0].Cells[0].Value = str;
                treeGridView1.Nodes[0].ImageIndex = 1;
                InvokeTreeGridViewImageIndex(treeGridView1, 1);
            }
        }

        private void InvokeTreeGridViewText(TreeGridView treeGridView, string text)
        {
            treeGridView.BeginInvoke((MethodInvoker)delegate
            {
                treeGridView.Nodes[0].Cells[0].Value = text;
            });
        }
        private void InvokeTreeGridViewImageIndex(TreeGridView treeGridView, int value)
        {
            treeGridView.BeginInvoke((MethodInvoker)delegate
            {
                treeGridView.Nodes[0].ImageIndex = value;
            });
        }
